...
import { getDailyOperationsReport, DailyOperationsReportData } from '@/services/salesDashboard';
...
export default function StoreDailyOperationsPage() {
  ...
  const handleExport = () => {
    ...
    const summaryRows = [
      ...
      { 'Chỉ số': 'Tổng số bill (TC)', 'Giá trị': data.totalBills },
    ...
    const dailyRows = data.rows.map((r) => {
      ...
      data.paymentTypes.forEach((pt, i) => { base[pt.name] = r.paymentAmounts[i] ?? 0; });
    ...
    exportSheetsToExcel('VanHanhTheoNgay_CuaHang', [
      {
        name: 'Tổng quan',
        rows: buildScopeHeaderRows({ reportName: 'Báo cáo vận hành theo ngày (Cửa hàng)', fromDate, toDate }),
      },
  ...
  const slotHeaderCls = 'px-3 py-2 text-right font-bold text-gray-500 dark:text-gray-400 uppercase whitespace-nowrap';
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto space-y-6 text-gray-800 dark:text-gray-100">
      ...
      <StoreDateFilter
        ...
        datePickerId="store-daily-operations-date-range"
      />
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-500 dark:text-gray-400">
          Chọn khoảng thời gian để xem báo cáo vận hành.
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <KpiCard label="Doanh thu gộp" value={formatVND(data.grossSales)} hint={`Sau giảm giá: ${formatVND(data.netSales)}`} icon={<DollarLineIcon className="w-4 h-4" />} />
            <KpiCard label="Lợi nhuận đến hiện tại" value={formatVND(data.profitToDate)} hint="Theo tháng hiện tại" icon={<DollarLineIcon className="w-4 h-4" />} accent={data.profitToDate >= 0 ? 'green' : 'red'} />
            <KpiCard label="Lợi nhuận dự kiến trong tháng" value={formatVND(data.projectedMonthlyProfit)} hint="Dự kiến cuối tháng" icon={<PieChartIcon className="w-4 h-4" />} accent={data.projectedMonthlyProfit >= 0 ? 'green' : 'red'} />
            <KpiCard label="Tổng bill (TC)" value={formatNumber(data.totalBills)} hint={`TB/bill (TA): ${formatVND(data.averageBill)}`} icon={<BoxCubeIcon className="w-4 h-4" />} accent="amber" />
          </div>
          ...
          <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4">
              ...
              <table className="min-w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-amber-50 dark:bg-amber-950/20 text-gray-700 dark:text-gray-200">
                    <th className="px-3 py-2 text-left font-bold" colSpan={2}>Tổng kỳ</th>
                    ...
                    {data.paymentTypes.map((pt) => (
                      <th key={pt.paymentTypeId} className={cellCls}>{formatCell(pt.total)}</th>
                    ))}
                  ...
                  <tr className="bg-gray-50 dark:bg-gray-800/50 border-y border-gray-200 dark:border-gray-700">
                    ...
                    {data.paymentTypes.map((pt) => (
                      <th key={pt.paymentTypeId} className={slotHeaderCls}>{pt.name}</th>
                    ))}
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {data.rows.map((r) => {
                    ...
                    return (
                      <tr key={r.date} className="hover:bg-gray-50/60 dark:hover:bg-gray-800/30">
                        <td className={`px-3 py-2 font-semibold ${isWeekend ? 'text-rose-600 dark:text-rose-400' : 'text-gray-700 dark:text-gray-200'}`}>{r.dayLabel}</td>
                        ...
                        {data.paymentTypes.map((pt, i) => (
                          <td key={pt.paymentTypeId} className={cellCls}>{formatCell(r.paymentAmounts[i] ?? 0)}</td>
                        ))}
            ...
            <p className="text-xs text-gray-400 mt-3">
              Cột khung giờ tính doanh thu thuần theo giờ tạo đơn. Cột thanh toán = tổng tiền theo từng phương thức.
