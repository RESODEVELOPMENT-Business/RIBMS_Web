...
export default function StoreOrdersReportPage() {
  ...
  const handleExport = () => {
    ...
    exportSheetsToExcel('BC2_DonHang_CuaHang', [
      {
        name: 'Tổng quan',
        rows: buildScopeHeaderRows({ reportName: 'BC#2 Báo cáo đơn hàng (Cửa hàng)', fromDate, toDate }),
        columnWidths: [22, 30],
      },
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 text-gray-800 dark:text-gray-100">
      ...
      <StoreDateFilter
        ...
        datePickerId="store-orders-report-date-range"
      />
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {data.dailyTrend.length === 0 ? (
                    ...
                    data.dailyTrend.map((d) => (
                      <tr key={d.date} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/20">
    ...
    <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-sm p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Tổng doanh thu: {formatVND(totalRevenue)}</p>
        </div>
      </div>

      <div className="h-10 mb-2 flex items-center justify-center">
        {hoveredData ? (
          <div className="flex items-center gap-4 bg-gray-50 dark:bg-gray-800 rounded-xl px-5 py-2 text-sm">
            <span className="font-semibold text-gray-700 dark:text-gray-200">
              {usingShifts ? '⏱' : '🕐'} {usingShifts ? hoveredData.label : `${hoveredData.label}:00 – ${hoveredData.label}:59`}
            </span>
            <span className="text-emerald-600 dark:text-emerald-400 font-medium">
              💰 {formatVND(hoveredData.revenue)}
            </span>
            <span className="text-blue-600 dark:text-blue-400">🧾 {hoveredData.invoiceCount} bill</span>
            <span className="text-amber-600 dark:text-amber-400">☕ {hoveredData.itemCount} ly/món</span>
          </div>
        ) : (
          <p className="text-xs text-gray-300 dark:text-gray-600 italic">Di chuột vào cột để xem chi tiết</p>
        )}
      </div>
            ...
            {dataItems.map((h) => {
              ...
              return (
                <div
                  ...
                  style={barWidthStyles as React.CSSProperties}
                  ...
                  >
                    {usingShifts && h.revenue > 0 && ratio > 25 && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-2 text-white pointer-events-none select-none z-10">
                        ...
                        <div className="text-[10px] font-medium opacity-90">🧾 {formatNumber(h.invoiceCount)} bill</div>
                        <div className="text-[10px] font-medium opacity-90 mt-0.5">☕ {formatNumber(h.itemCount)} món</div>
                      </div>
                    )}
                  </div>
